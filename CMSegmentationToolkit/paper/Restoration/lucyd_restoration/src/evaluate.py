...
from CMSegmentationToolkit.paper.Restoration.lucyd_restoration.src.ssim import get_SSIM
...
def predict(model, img_blurred):
    model.eval()
    img_blurred = img_blurred.to(device)
    y_hat, y_k, update = model(img_blurred.float())
    return y_hat