...
def prepare_masks(path_to_instance_segmentation, path_to_image, output_path_img, output_path_mask):
    ...
    combined_mask = np.zeros(shape=(segmentation.shape), dtype=np.uint8)
    combined_mask[dilated_segmentation > 0] = 2
    combined_mask[segmentation > 0] = 1
